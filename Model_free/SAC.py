...
class SAC_Policy:
    def __init__(self, env, load_PI=False):
        ...
        if load_PI:
            self.actor = Actor(input_dim=env.observation_space.shape[0], output_dim=env.action_space.shape[0], l_r=self.hyperparameters["l_r"])
            self.actor.load_state_dict(torch.load('Model_free/output/SAC_NN.pth'))
        else:
            self.actor = Actor(input_dim=env.observation_space.shape[0], output_dim=1, l_r=self.hyperparameters["l_r"])
            self.critic = Critic(input_dim=env.observation_space.shape[0]+env.action_space.shape[0], output_dim=1, l_r=self.hyperparameters["l_r"])
            self.critic_target = Critic(input_dim=env.observation_space.shape[0]+env.action_space.shape[0], output_dim=1, l_r=self.hyperparameters["l_r"])
    ...
    def learn(self):
        ...
        print("Saving Q")
        torch.save(self.actor.state_dict(), 'Model_free/output/SAC_NN.pth')
    ...
    def train(self):
        for _ in range(self.hyperparameters["epochs"]):
            states, actions, next_states, rewards, dones = self.buffer.sample(self.hyperparameters["batch_size"], "torch") 
            ...
            #### Alpha train ####
            sample, log_prob = self.actor.sample(states)
            _, log_prob = self.clip_actor(sample, log_prob) # need to repeat it because self.actor.optimizer.zero_grad() removes log_prob gradient graph
    ...
    def clip_actor(self, sample, log_prob):
        ...
        log_prob -= torch.sum(torch.log(self.action_scale*(1-sample.pow(2)) + 1e-6), dim=-1, keepdim=True)
        return action, log_prob
    ...
    def get_target(self, x_next, r, done):
        with torch.no_grad():
            ...
            target = r.unsqueeze(1) + (~done.unsqueeze(1)) * self.hyperparameters["discount_factor"]*(q_target + entropy)
        return target
